...
namespace Jellyfin.Plugin.YoutubeMetadata.Providers
{
    ...
    public class YTDLSeriesProvider : AbstractYoutubeRemoteProvider<YTDLSeriesProvider, Series, SeriesInfo>
    {
        ...
        public override async Task<MetadataResult<Series>> GetMetadata(SeriesInfo info, CancellationToken cancellationToken)
        {
            ...
            var cacheKey = Utils.ToSafeCacheKey(name);
            var ytPath = GetVideoInfoPath(this._config.ApplicationPaths, cacheKey);
            ...
            {
                _logger.LogDebug("YTDLSeries GetMetadata: No channel metadata cache for {Name}", name);
                result.HasMetadata = false;
                return result;
            }
        ...
        internal override MetadataResult<Series> GetMetadataImpl(YTDLData jsonObj, string id) =>
            Utils.YTDLJsonToSeries(jsonObj, Plugin.Instance?.Configuration, jsonObj.uploader);
            ...
            CancellationToken cancellationToken)
        {
            ...
            var cacheKey = Utils.ToSafeCacheKey(searchQuery);

            _logger.LogDebug("YTDLSeries GetMetadataImpl: GetAndCacheMetadata {Name}", searchQuery);
            var ytPath = GetVideoInfoPath(this._config.ApplicationPaths, cacheKey);
            var fileInfo = _fileSystem.GetFileSystemInfo(ytPath);
            if (!IsFresh(fileInfo))
            {
                ...
                try
                {
                    var channelId = await Utils.SearchChannel(searchQuery, appPaths, cancellationToken).ConfigureAwait(false);
                    if (!string.IsNullOrWhiteSpace(channelId))
                    {
                        await Utils.GetChannelInfo(channelId, cacheKey, appPaths, cancellationToken).ConfigureAwait(false);
                    }
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(ex, "YTDLSeries GetMetadataImpl: Failed to fetch channel info for {Name}", searchQuery);
                }
            }
        ...
        public override async Task<IEnumerable<RemoteSearchResult>> GetSearchResults(SeriesInfo searchInfo, CancellationToken cancellationToken)
        {
            ...
            if (searchInfo.ProviderIds != null &&
                searchInfo.ProviderIds.TryGetValue(Constants.PluginName, out var channelId) &&
            ...
            var searchResults = await Utils.SearchChannels(searchQuery, this._config.ApplicationPaths, cancellationToken, maxResults);
