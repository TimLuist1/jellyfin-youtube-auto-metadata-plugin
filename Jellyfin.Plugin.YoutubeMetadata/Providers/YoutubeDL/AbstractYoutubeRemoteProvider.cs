...
namespace Jellyfin.Plugin.YoutubeMetadata.Providers
{
        ...
        where E : ItemLookupInfo, new()
    {
        ...
        public virtual async Task<MetadataResult<T>> GetMetadata(E info, CancellationToken cancellationToken)
        {
            ...
            {
                if (pluginConfig?.EnableTitleLookupWithoutId == true)
                {
                    var searchQuery = Utils.BuildSearchQuery(info.Name, info.Path);
                    _logger.LogDebug("YTDL GetMetadata: No ID found, searching by title '{Title}'", searchQuery);

                    YTSearchResult bestMatch = null;
                    try
                    {
                        bestMatch = await Utils.SearchBestVideoMatch(
                            searchQuery,
                            this._config.ApplicationPaths,
                            cancellationToken,
                            pluginConfig.SearchResultLimit).ConfigureAwait(false);
                    }
                    catch (Exception ex)
                    {
                        _logger.LogWarning(ex, "YTDL GetMetadata: Title search failed for '{Title}'", searchQuery);
                    }

                    if (bestMatch is not null)
                    {
                        id = bestMatch.Id;
                        _logger.LogDebug("YTDL GetMetadata: Matched title '{Title}' to YouTube ID {ID}", searchQuery, id);
                    }
                }

                if (string.IsNullOrWhiteSpace(id))
                {
                    _logger.LogInformation("YTDL GetMetadata: Youtube ID not found for title: {Name}", info.Name);
            ...
            {
                _logger.LogWarning("YTDL GetMetadata: Metadata cache not found for ID={ID}", id);
                result.HasMetadata = false;
                return result;
            }
            ...
            if (video != null)
            {
                ...
                await TryRefineMetadataWithAi(video, cancellationToken).ConfigureAwait(false);
        ...
        public virtual async Task<IEnumerable<RemoteSearchResult>> GetSearchResults(E searchInfo, CancellationToken cancellationToken)
        {
            ...
            if (searchInfo.ProviderIds != null &&
                searchInfo.ProviderIds.TryGetValue(Constants.PluginName, out var youtubeId) &&
                ...
                try
                {
                    ...
                    if (video != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(video.upload_date))
                        {
                            try
                            {
                                var date = DateTime.ParseExact(video.upload_date, "yyyyMMdd", CultureInfo.InvariantCulture);
            ...
            var maxResults = Plugin.Instance?.Configuration?.SearchResultLimit ?? 10;
            var searchResults = await Utils.SearchVideos(searchQuery, this._config.ApplicationPaths, cancellationToken, maxResults);
        ...
        private async Task TryRefineMetadataWithAi(YTDLData video, CancellationToken cancellationToken)
        {
            var pluginConfig = Plugin.Instance?.Configuration;
            if (pluginConfig == null || !pluginConfig.EnableAiMetadataCleanup)
            {
                return;
            }

            var refined = await AiMetadataRefiner.TryRefineAsync(
                video.title,
                video.description,
                pluginConfig,
                _httpClientFactory.CreateClient(Constants.PluginName),
                _logger,
            ...
            if (!string.IsNullOrWhiteSpace(refined.Value.Title))
            {
                video.title = refined.Value.Title;
            }

            if (pluginConfig.EnableAiDescriptionCleanup && !string.IsNullOrWhiteSpace(refined.Value.Description))
            {
                video.description = refined.Value.Description;
            }
        }
    }
}
