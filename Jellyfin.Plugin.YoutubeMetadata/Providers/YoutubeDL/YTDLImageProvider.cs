    ...
    public class YTDLImageProvider : IRemoteImageProvider, IHasOrder
    {
        ...
        public async Task<IEnumerable<RemoteImageInfo>> GetImages(BaseItem item, CancellationToken cancellationToken)
        {
            ...
            if (string.IsNullOrWhiteSpace(id))
            {
                ...
                var searchQuery = Utils.BuildSearchQuery(item.Name, item.Path);
                ...
                {
                    _logger.LogWarning(ex, "YTDLImage GetImages: title search failed for {Name}", item.Name);
                }
                id = bestMatch?.Id;

                if (string.IsNullOrWhiteSpace(id))
                {
                    _logger.LogDebug("YTDLImage GetImages: No search result for title: {Name}", item.Name);
                    return result;
                }
            }
